...
const ContactForm = () => {
    ...
    const sendEmail = e => {
        e.preventDefault();
        emailjs.sendForm('service_2cpaksc', 'portfolio_fojptya', e.target, 'user_uzAdPfmx6TBIUFL6PqG5Q')
            .then((result) => {
    ...
    return (
        <div className=''>
            <form onSubmit={sendEmail}>
                ...
                <p style={{color: email ? 'green': 'red', fontWeight:'bold'}}>{email && 'Successfully send your valuable message..'}</p>
                <p style={{color: email===false && 'red', fontWeight:'bold'}}>{email===false && 'Please Try again..'}</p>
            </form>
        </div>
    );
};
